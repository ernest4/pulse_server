  ...
  module Ecs
    module Systems
      class SpatialPartitioning < Fast::ECS::System
        ...
        def update
          engine.query(Component::Location, Component::Transform) do |query_set|
            location, transform = Tuple(Component::Location, Component::Transform).from(query_set)
        ...
        private def update_or_create_nearby_characters_component(location, transform)
          ...
          current_map.nearby_characters(transform.position.x, transform.position.y) do |character_entity_id|
            unless character_entity_id == transform.id
              updated_nearby_set.add(SparseSet::Item.new(character_entity_id))
            end
          end
        ...
        private def find_or_create_nearby_characters_component(entity_id)
          nearby_characters = engine.get_component(Component::NearbyCharacters, entity_id)
          return create_nearby_characters_component(entity_id) if nearby_characters.nil?

          nearby_characters.as Component::NearbyCharacters
        end

        private def create_nearby_characters_component(entity_id)
          nearby_characters = Component::NearbyCharacters.new(entity_id: entity_id)
